                ...
                <div style="horizontal-align:top" class="row">
                    <div class="col-lg-4 margin_top_30">
                        <div class="full margin_top_30">
                            <h3 style="font-size:35px" class="main_heading _left_side margin_top_30">Selected Vehicle</h3>
                    ...
                    <div style="border: 2px solid #161c34; border-radius:5px;margin-top: 60px" class="col-lg-8">
                        <div class="full">
                            <div class="contact_form">
                                <form action="/bookings/update" method="post" id="bookingform">
                                    <fieldset class="row">
                                        <div class="col-md-12">
                                            <div class="full">
                                                <div class="row">
                                                    <div style="width:48%;margin-right: 2%" class="column field">
                                                        ...
                                                        <input value="${booking.startDate}" type="date" id="startDate"
                                                               name="startDate"/>
                                                    </div>
                                                    <div style="width:48%;margin-left:2%" class="column field">
                                                        Date of Return
                                                        <input value="${booking.bookingId}" name="bookingId"
                                                               id="bookingId" type="hidden"/>
                                                        <input value="${booking.endDate}" type="date" id="endDate"
                                                               name="endDate"/>
                                                    </div>
                                                    <c:set var="size" value="${bookedEq.size()}"/>
                                                    <c:choose>
                                                        <c:when test="${size>0}">
                                                            <div class="col-md-12">
                                                                Reserved Equipment
                                                            </div>
                                                            <c:forEach var="equip" items="${bookedEq}">
                                                                <div class="col-md-12">
                                        ...
                                        <div style="margin-top: 15px" class="col-md-12">
                                                <div class="row col-md-12">
                                                    <c:forEach var="equiumpent" items="${equipment}">
                                                        <c:url value="/bookings/addEquipment" var="addEq">
                                                            <c:param name="equipmentId"
                                                                     value="${equiumpent.equipmentId}"/>
                                                            <c:param name="bookingId" value="${booking.bookingId}"/>
                                                        </c:url>
                                                        <div style="width:30%;margin-right:2%" class="column field">
                                                            <a href="${addEq}">
                                                                <button type="button" style="background-color: #0f0824"
                                                                        class="btn btn-primary">
                                                                    Add ${equiumpent.equipmentName}</button>
                                                            </a>
                                                        </div>
                                                    </c:forEach>
                                                </div>
                                        </div>
                                        <div style="margin-top: 15px;margin-left: 7px" class="row col-md-12">
                                            <div class="column">
                                                        <button onclick="submitForm()" id="updateBtn" class="submit_bt">
                                                            Update
                                                        </button>
                                            </div>
                                            <div style="margin-left: 10px" class="column">
                                                <c:url value="/bookings/delete" var="delete">
                                                    <c:param name="bookingId" value="${booking.bookingId}"/>
                                                </c:url>
                                                <a data-method="delete" href="${delete}" methods="delete">
                                                    <button type="button" class="submit_bt">Cancel Booking</button>
                                                </a>
                                            </div>
                                        </div>
                                    </fieldset>
...
<script>
    ...
    window.onLoad = $(function () {
        if (new Date("${booking.startDate}") <= new Date()) {
            ...
            window.alert("You Cannot Update this booking. Date of Collection has already passed");
...
<script>
    function submitForm() {
        ...
        element.addEventListener('submit', event => {
            ...
            array.push("${it.startDate}");
            array.push("${it.endDate}");
            </c:forEach>
            ...
            if (diffDays > 14) {
                ...
                if (sDate > eDate) {
                    ...
                    if (bookable) {
                        window.alert("Your Reservation has been updated");
